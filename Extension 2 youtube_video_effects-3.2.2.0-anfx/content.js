...
function applyFilter(index, manual) {
	...
	//var x = document.querySelectorAll('#movie_player .html5-video-container');//console.log(videos.length);
	if (videos.length > 0) {
		var video = videos[0];
		...
		if (filter != null && filter != undefined) {
			if (index < len) {
				...
				y.style.filter = '';
			} else if(index==57){//console.log(manual);
				filter.style.background = "";
				filter.style.backgroundSize = 'unset';
...
chrome.runtime.onMessage.addListener(
	function (request, sender, sendResponse) {
		if (request.youtube_filter_selected_index !== undefined) {
			...
			//console.log(selected = request.youtube_filter_selected_index+", "+request.youtube_video_filters_manual);
			var _thread_apply_filter = setInterval(function(){
				if (document.getElementById('ytp-video-filters-htcom') != null){
					clearInterval(_thread_apply_filter);//console.log(selected+manual);
					applyFilter(selected, manual);
				}
			}, 50);
			//setTimeout(function(){applyFilter(selected, manual);}, 1000);
		}
